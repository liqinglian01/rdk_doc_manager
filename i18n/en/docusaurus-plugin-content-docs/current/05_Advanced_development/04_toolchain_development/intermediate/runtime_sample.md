...
The following example logs are from testing on the **RDK X3** development board.
:::
...
The following logs are from testing on the **RDK X3** development board.
:::
...
Please use the `build_ptq_xj3.sh` script under the `code` directory to one-click compile the executable program for the development board. The executable program and its dependencies are automatically copied to the `aarch64` directory under `xj3/ptq/script`.

:::info Note
  The cross-compilation toolchain path in `build_ptq_xj3.sh` defaults to `/opt`. If you installed the toolchain elsewhere, edit the script accordingly.
:::
```shell
...
The following examples are based on testing on the **RDK X3** development board.
:::
...
The following examples are test results from the **RDK X3** development board.
:::
...
:::caution Note
  ...
  The following example is based on test results from the **RDK X3** development board.
:::
...
```bash
  ...
  # For other platforms, adjust paths to match your environment:
  |-- ptq
  |   |-- data
  |   |   |-- cityscapes
...
:::caution Note
  ...
  The following example shows actual test results from the **RDK X3** development board.
:::
...
:::caution Note
  ...
  The following example is based on the **RDK X3** development board.
:::
...
:::caution Note
  ...
  The following example uses the **RDK X3** development board.
:::
...
The model information includes the model's input and output information ``hbDNNTensorProperties`` and the model's segment information ``stage``. The segment information means an image can be inferred in multiple stages; ``stage`` is [x1, y1, x2, y2], the top-left and bottom-right coordinates of the inference region. Some BPU architectures (such as **BAYES / BAYES_E**) support such segmented models; models on **RDK X3** are all single-stage.
...
The above example shows output logs from the **RDK X3** development board; for other boards, refer to your actual logs.
