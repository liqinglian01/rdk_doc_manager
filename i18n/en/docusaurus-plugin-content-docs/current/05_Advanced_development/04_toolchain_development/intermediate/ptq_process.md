...
:::tip Tips
  ...
  It is recommended to refer to the script methods in the D-Robotics ``horizon_model_convert_sample`` package for caffe, onnx, and other examples (for instance, ``01_check_X3.sh``; use the check script that matches your target chip if the package provides one).
:::
...
--march
  Specifies the D-Robotics processor type to be adapted, such as ``bernoulli2``, ``bayes``, or ``bayes-e``; use ``bernoulli2`` for **RDK X3**, ``bayes-e`` for **RDK X5**, and other values as required by your chip and toolchain deliverables.
...
- The following ONNX model running on a **BAYES architecture platform** has a structure of Mul + Add + Mul. From the operator constraint list for **BAYES architecture platforms**, Mul and Add operators are supported on five dimensions for BPU execution, but they must meet the **BAYES** BPU operator constraints; otherwise, they will fall back to CPU computation.
...
To avoid excessive code length, the implementation code of various simple transformers is not included. For specific usage, please refer to the chapter "Transformer Usage" in the [**Toolchain**](../../../06_FAQ/05_toolchain.md#transposetransformer) section.
...
- Run BPU executable operators on the BPU as much as possible (if using **RDK X5**, you can specify operators to run on the BPU through the ``node_info`` parameter in the YAML file; **RDK X3** is automatically optimized and cannot specify operators through the YAML configuration file).
...
:::tip Tips
  ...
  It is recommended to refer to the script methods of the example models in the D-Robotics Model Conversion "horizon_model_convert_sample" package, such as caffe and onnx example models: "03_build_X3.sh" (or the build script for your target platform if the package provides one).
:::
...
--march
Microarchitecture of BPU. Set it to "bernoulli2" if using **RDK X3**, and set it to "bayes-e" if using **RDK X5**; for other platforms, choose ``march`` according to the toolchain documentation.

:::caution Note

- For "RDK X3 yaml configuration file", fill in the template file [**RDK X3 Caffe model quantization yaml template**](../../../06_FAQ/05_toolchain.md#rdk_x3_caffe_yaml_template) or [**RDK X3 ONNX model quantization yaml template**](../../../06_FAQ/05_toolchain.md#rdk_x3_onnx_yaml_template) directly.

- For **RDK X5** YAML configuration files, use the [**RDK X5 Caffe model quantization yaml template**](../../../06_FAQ/05_toolchain.md#rdk_x5_caffe_yaml_template) and [**RDK X5 ONNX model quantization yaml template**](../../../06_FAQ/05_toolchain.md#rdk_x5_onnx_yaml_template) directly.

- If the hb_mapper makertbin step terminates abnormally or shows an error message, it means that the model conversion has failed. Please check the error message and modification suggestions in the terminal printout or in the ``hb_mapper_makertbin.log`` log file generated in the current path. You can find the solution for the error in the [**Model Quantization Errors and Solutions**](../../../06_FAQ/05_toolchain.md#model_convert_errors_and_solutions) section. If the problem cannot be solved after these steps, please contact the D-Robotics technical support team or submit your question in the [**D-Robotics Official Technical Community**](https://developer.d-robotics.cc/). We will provide support within 24 hours.
...
```
  ...
  model_parameters:
    ...
    # Target processor architecture: RDK X3 uses bernoulli2 (march: 'bernoulli2'), RDK X5 uses bayes-e (march: 'bayes-e')
    march: 'bernoulli2'
...
- When configuring ``march`` as `bayes` (model conversion for the `bayes` architecture), if you set the optimization level ``optimize_level`` to O3, `hb_mapper makerbin` enables caching by default: the first compile creates a cache file; later compiles in the same working directory reuse it and shorten compile time.
- When configuring ``march`` as `bayes-e` (**RDK X5** model conversion), if you set ``optimize_level`` to O3, `hb_mapper makerbin` provides the same caching behavior.
:::
...
| `march`       | **Purpose:** Specifies the platform architecture supported by the mixed-heterogeneous model to be produced.<br/>**Description:** The available values correspond to the BPU micro-architecture for platforms such as RDK X3. Choose based on your platform and deliverables. | `bernoulli2` or `bayes` | Required          |
...
| `set_node_data_type` | **Purpose:** Configures the output data type of a specific op as int16 — **only on platforms that support int16/node_info (such as RDK X5).**<br/>**Description:** In the model conversion process, most ops default to int8 for input and output data types. This parameter allows you to specify the output data type of a specific op as int16 under certain constraints. See the [int16 configuration details](#int16_config) for more information.<br/>**Note:** This functionality has been merged into the `node_info` parameter, which will be deprecated in future versions. | Supported operators: see the [model operator support list](./supported_op_list) for **BAYES/BAYES_E** and **RDK X5** constraints. | Optional          |
...
| `run_on_cpu` | Forces operators to run on CPU.<br/>**Description**: Although CPU performance is inferior to BPU, it provides float precision calculations. Specify this parameter if you're certain that some operators need to run on CPU. Set values to specific node names in your model, following the same configuration method as described earlier for `param_value`. **Note**: On **RDK X5**, this parameter has been merged into the `node_info` parameter and is planned to be deprecated; **RDK X3** still uses it. | N/A | Optional |
| `run_on_bpu` | Forces an operator to run on BPU.<br/>**Description**: To maintain the accuracy of the quantized model, occasionally, the conversion tool may place some operators that can run on BPU on CPU. If you have higher performance requirements and are willing to accept slightly more quantization loss, you can explicitly specify that an operator runs on BPU. Set values to specific node names in your model, following the same configuration method as described earlier for `param_value`. **Note**: On **RDK X5**, this parameter has been merged into the `node_info` parameter and is planned to be deprecated; **RDK X3** still uses it. | N/A | Optional |
| `optimization` | Sets the model output format to int8 or int16.<br/>**Description**: If set to `set_model_output_int8`, the model will output in low-precision int8 format; if set to `set_model_output_int16`, the model will output in low-precision int16 format. **Note**: **RDK X3** only supports `set_model_output_int8`; **RDK X5** supports `set_model_output_int8` and `set_model_output_int16`. | `set_model_output_int8`, `set_model_output_int16` | Optional |
...
If you are using the **RDK X5** product, you can also try precision tuning by configuring some ops to use int16 (**RDK X3** does not support int16 for ops):
...
For **RDK X5**, the algorithm toolchain supports specifying certain ops to compute in int16, as described in [**int16 configuration**](#int16_config). By configuring ops that are sensitive to quantization loss (using cosine similarity as a reference) to use int16, accuracy loss can be reduced in some scenarios.
:::
...
For the current version of the accuracy debug tool: **BAYES architecture platforms** (`bayes` models) support both command line and API debugging; **RDK X5** (`bayes-e` models) support both command line and API debugging; **RDK X3** (`bernoulli2` models) only support API debugging.
:::
...
:::caution Note
  ...
  In the current toolchain version, `runall` is only applicable to **RDK X5**.
:::
...
Based on previous optimization experience, this strategy covers most scenarios. If issues persist, follow the [**Precision Optimization Checklist**](../../../06_FAQ/05_toolchain.md#checklist) to gather detailed model configuration information, ensure all troubleshooting steps have been completed, and report the filled checklist, the original float model file, and relevant configuration files to the D-Robotics support team or the [**D-Robotics Official Technical Community**](https://developer.d-robotics.cc/) for further assistance.
