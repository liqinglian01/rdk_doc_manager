...
2. Set `march`. Set `bernoulli2` for **RDK X3** and set `bayes-e` for **RDK X5**.
...
Currently, only **RDK X5** with the BPU architecture set to `BAYES_E` supports setting `int16` quantization.
:::
...
# Set march: **RDK X3** to BERNOULLI2, **RDK X5** to BAYES_E.
...
# **RDK X3** sets BERNOULLI2, **RDK X5** sets BAYES_E.
set_march(March.BAYES)
...
# **RDK X3** set BERNOULLI2, **RDK X5** set BAYES_E
set_march(March.BAYES)
...
# **RDK X3** set BERNOULLI2， **RDK X5** set as BAYES_E。
set_march(March.BAYES)
...
Currently, only **RDK X5** with BPU architecture set to `BAYES_E` supports setting `int16` quantization.
:::
...
Currently, only **RDK X5** with BPU architecture set to "BAYES_E" supports "int16" quantization.
:::
...
# Set BERNOULLI2 for **RDK X3**, Set BAYES_E for **RDK X5**.
set_march(March.BAYES)
...
# Set BERNOULLI2 for **RDK X3** and BAYES_E for **RDK X5**.
set_march(March.BAYES)
...
# **RDK X3** set BERNOULLI2, **RDK X5** set to BAYES_E.
set_march(March.BAYES)
shape = np.random.randint(10, 20, size=4).tolist()
...
Parameters:
    ...
    march: BPU architecture to use. If None, it will automatically use get_march() to get the current specified architecture.
        Default is None. For **RDK X3**, set to BERNOULLI2, for **RDK X5**, set to BAYES_E.
...
2. Find layers with quantization abnormalities using Debug Tools
   ...
   - Model Input
     ...
     Currently, only platforms with BPU architecture `BAYES` or `BAYES_E` support setting `int16` quantization.
     :::
    ...
    :::caution Caution
        ...
        Currently, only **RDK X5** with BPU architecture `BAYES_E` supports setting `int16` quantization.
