...
```bash
    # Check the model structure and operator support, and provide hardware allocation for each operator (BPU/CPU). Example (RDK X3): 01_check_X3.sh
    bash 01_check_X3.sh
```
...
```bash
    # The configuration file yolov5s_config_X3.yaml for conversion is in the same folder as 03_build_X3.sh. Example (RDK X3): 03_build_X3.sh
    bash 03_build_X3.sh
```
...
-   cache.json                              # Cache file (auto-generated when RDK X5 conversion uses `optimize_level` O3 with `march` `bayes-e`)
