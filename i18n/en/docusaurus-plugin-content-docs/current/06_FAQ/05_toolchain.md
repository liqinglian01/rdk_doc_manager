...
### Q5: The algorithm toolchain Docker image is based on Ubuntu 20.04. Will this affect running the generated models (.bin or .hbm) on RDK boards with Ubuntu 22.04?
...
### Q6: How do I deploy YOLO series models (e.g., YOLOv5, YOLOv8, YOLOv10) on the RDK platform?
...
* **YOLOv10 on Bayes architecture platforms (such as RDK X5):**
...
### Q7: When deploying YOLOv5, I get an error like `can't reshape xxx in (84,84,3,85)`. How do I fix this?
...
### Q8: When deploying YOLOv5, I get a large number of irregular detection boxes. Why?
...
### Q9: When deploying YOLOv5, detection boxes appear in a periodic pattern. Why?
**A:**
...
### Q10: When deploying YOLOv5, detection boxes are offset from their correct positions. Why?
**A:**
...
### Q11: When deploying YOLOv5, all detection boxes cluster in the top-left corner. Why?
**A:**
...
### Q12: When running `/app/pydev_demo/07_yolov5_sample` with my own model, I get a `Segmentation fault`. What should I do?
**A:**
...
### Q13: If the model inference produces no results or results are far from expected, how should I troubleshoot (pipeline checklist)?
**A:** 
...
### Q14: How can I obtain board-side `hrt_*` performance analysis tools (e.g., `hrt_model_exec`, `hrt_bpu_monitor`)?
...
Use the `hb_model_verifier` tool to ensure ../05_Advanced_development between the quantized.onnx and .bin files, with model outputs aligned to at least two or three decimal places.
...
If the model ../05_Advanced_development check passes, carefully examine the board-side preprocessing and post-processing code!

In case of a failure in the ../05_Advanced_development check between the quantized.onnx and .bin models, contact D-Robotics technical support.

#### Quantization YAML Configuration File Templates

##### RDK X3 Caffe Model Quantization YAML Template {#rdk_x3_caffe_yaml_template}

Create a `caffe_config.yaml` file and copy the following content, then fill in the marked **`required parameters`** to proceed with model conversion. For more information on parameter usage, see the "[YAML Configuration File Explanation](../05_Advanced_development/04_toolchain_development/intermediate/ptq_process#yaml_config)" chapter.
```yaml
